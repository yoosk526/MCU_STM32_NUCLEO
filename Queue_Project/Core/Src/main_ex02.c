...
int main(void)
{
  ...
  /* Create the queue(s) */
  /* creation of Queue2 */
  Queue2Handle = osMessageQueueNew (10, sizeof(uint16_t), &Queue2_attributes);
...
void StartSender1(void *argument)
{
  ...
  for(;;)
  {
	  osStatus_t st = osMessageQueuePut(Queue2Handle, &value, 0, 0);
	  if (st == osOK)
	  {
		  snprintf(msg, sizeof(msg), "Sent : %u \r\n", (unsigned int)value);
		  uart_print(msg);
		  value++;
	  }
	  osDelay(700);
